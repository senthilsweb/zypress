...
REPO_NAME="https://github.com/senthilsweb/zypress.git"
OS="$(uname -s)"
GIT_COMMIT=$(git rev-parse --short HEAD)
TAG=$(git describe --exact-match --abbrev=0 --tags ${COMMIT} 2> /dev/null || true)
TAG=${TAG:="prod"}
DATE=$(date +'%Y-%m-%d')
APP_NAME='zypress-server'
BIN_NAME=$APP_NAME'_'
DIST='./release/'
DEST='./.temp/'
...
echo go build -ldflags "-X $REPO_NAME/cmd.GitCommit=$GIT_COMMIT -X $REPO_NAME/cmd.Version=$TAG -X $REPO_NAME/cmd.BuildDate=$DATE" -o $DEST$BIN_NAME'mac_amd64' -v .
